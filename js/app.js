...
const db = db_client.db("FSbot")
const EventDB = db.collection("event");
const MsgTable = db.collection("msg");
const OpenAIApi = require('openai');


const FEISHU_APP_ID = process.env.FS_APPID || ""; // 飞书的应用 ID
const FEISHU_APP_SECRET = process.env.FS_SECRET || ""; // 飞书的应用的 Secret
const FEISHU_BOTNAME = process.env.FS_BOTNAME || ""; // 飞书机器人的名字
const OPENAI_KEY = process.env.HUNYUAN_API_KEY || ""; // OpenAI 的 Key
const OPENAI_MODEL = process.env.MODEL || "hunyuan-pro"; // 使用的模型
...
async function buildConversation(sessionId, question) {
  ...
  const historyMsgs = await MsgTable.find({ sessionId }).toArray();
...
async function saveConversation(sessionId, question, answer) {
  ...
  const result = await MsgTable.insertOne({
    sessionId,
    question,
    answer,
    msgSize,
...
async function discardConversation(sessionId) {
  let totalSize = 0;
  const countList = [];
  const historyMsgs = await MsgTable.find({ sessionId }).sort({ createdAt: -1 });
...
async function handleReply(userInput, sessionId, messageId, eventId) {
  ...
  await EventDB.findOneAndUpdate(
    { event_id: eventId }, // 查询条件
    { $set: { content: userInput.text } }, // 更新内容
    { new: true, upsert: true } // 选项：返回更新后的文档，如果不存在则插入新文档
  );
